...
class Onepoint extends Component{
  ...
  data(){
    axios.get('http://localhost:4000/onepoint').then(res => {
      this.setState({
        fx: res.data[0].fx,
        x0: res.data[0].x0,
        showInput: false,
      ...
      this.forceUpdate();
      this.Onepoint(Number(this.state.x0));
  ...
  render(){
    ...
    return(
      ...
      <div className="Onepoint">
        ...
        <br />
        <br />
        <h1 style={{textAlign:'center'}}>Onepoint Method</h1>
        <br />
        <Col sm='20' md={{ size: 6, offset: 3 }}>
          ...
          {this.state.showBotton && <Button href = '/Root_of_Equation/Onepoint' type="button" class="btn btn-primary btn-lg">Start agains</Button>}
        </Col>
        </Layout>
      </div>
      );
