...
class False_Position extends Component{
  data(){
    axios.get('http://192.168.99.100:8080/falseposition').then(res => {
      this.setState({
        fx: res.data[0].fx,
        xl: res.data[0].xl,
        xr: res.data[0].xr,
  ...
  render(){
    ...
    return(
      ...
      <div className="False_Position">
        ...
        <br />
        <br />
        <h1 style={{textAlign:'center'}}>False_Position Method</h1>
        <br />
        <Col sm='20' md={{ size: 6, offset: 3 }}>
          ...
          <Input type='text' name='fx' placeholder='x^5-4' value={this.state.fx} onChange={this.handleChange} />
          </InputGroup>
          ...
          <Input type="text" class="form-control" name='xl' placeholder='1.75' onChange={this.handleChange}/>
          <Input type="text" class="form-control" name='xr' placeholder='6.26' onChange={this.handleChange}/>
          <InputGroupAddon addonType='prepend'>
          ...
          {this.state.showBotton && <Button href = '/Root_of_Equation/False_Position' type="button" class="btn btn-primary btn-lg">Start agains</Button>}
