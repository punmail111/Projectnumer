...
import { Interpalation } from '../components/Interpalation';
...
/*data(){
    axios.get('http://localhost:4000/Newton').then(res => {
      ...
      this.forceUpdate();
      this.Newton(Number(this.state.xl),Number(this.state.xr));
  ...
  handleSubmit(event) {
    ...
    this.Newton(parseFloat(this.state.xl),parseFloat(this.state.xr));
    this.setState({
      showInput: false
    })
  }
      ...
      <div className="Newton">
        ...
        <br />
          <Col sm='20' md={{ size: 6, offset: 3 }}>
              ...
              {this.state.showBotton && <Button href = '/Interpalation/Newton' type="button" class="btn btn-primary btn-lg">Start agains</Button>}
            </Col>
